...
void tuForward(stuStruct *stu, ChromaSub chromaSub)
{
	...
	tuForward(stu->hscu->scbLuma, CU_SIZE / 8, CU_SIZE);
	tuForward(stu->vscu->scbLuma, CU_SIZE, CU_SIZE / 8);

	// Process the chroma components
	if (chromaSub == CHROMA_444)
	{
		tuForward(stu->hscu->scbChroma1, CU_SIZE / 8, CU_SIZE);
		tuForward(stu->hscu->scbChroma2, CU_SIZE / 8, CU_SIZE);
		tuForward(stu->vscu->scbChroma1, CU_SIZE, CU_SIZE / 8);
		tuForward(stu->vscu->scbChroma2, CU_SIZE, CU_SIZE / 8);
	}
	else if (chromaSub == CHROMA_420)
	{
		tuForward(stu->hscu->scbChroma1, CU_SIZE / 16, CU_SIZE / 2);
		tuForward(stu->hscu->scbChroma2, CU_SIZE / 16, CU_SIZE / 2);
		tuForward(stu->vscu->scbChroma1, CU_SIZE / 2, CU_SIZE / 16);
		tuForward(stu->vscu->scbChroma2, CU_SIZE / 2, CU_SIZE / 16);
	}
}
...
void tuForward(s16 *stb, u8 width, u8 height)
{
	...
	for (u8 y = 0; y < height; y++)
	{
		...
		{
			v0 = (s32)stb[y * CU_SIZE + x + 0] + (s32)stb[y * CU_SIZE + x + 3];
			v1 = (s32)stb[y * CU_SIZE + x + 1] + (s32)stb[y * CU_SIZE + x + 2];
			v2 = (s32)stb[y * CU_SIZE + x + 2] - (s32)stb[y * CU_SIZE + x + 1];
			v3 = (s32)stb[y * CU_SIZE + x + 3] - (s32)stb[y * CU_SIZE + x + 0];
			stb[y * CU_SIZE + x + 0] = coeffCast((64 * (v0 + v1)) >> 1);
			stb[y * CU_SIZE + x + 1] = coeffCast((-36 * v2 - 83 * v3) >> 1);
			stb[y * CU_SIZE + x + 2] = coeffCast((64 * (v0 - v1)) >> 1);
			stb[y * CU_SIZE + x + 3] = coeffCast((83 * v2 - 36 * v3) >> 1);
		}
	}
	...
	for (u8 y = 0; y < height; y += 4)
	{
		...
		{
			v0 = (s32)stb[(y + 0) * CU_SIZE + x] + (s32)stb[(y + 3) * CU_SIZE + x];
			v1 = (s32)stb[(y + 1) * CU_SIZE + x] + (s32)stb[(y + 2) * CU_SIZE + x];
			v2 = (s32)stb[(y + 2) * CU_SIZE + x] - (s32)stb[(y + 1) * CU_SIZE + x];
			v3 = (s32)stb[(y + 3) * CU_SIZE + x] - (s32)stb[(y + 0) * CU_SIZE + x];
			stb[(y + 0) * CU_SIZE + x] = coeffCast((64 * (v0 + v1)) >> 8);
			stb[(y + 1) * CU_SIZE + x] = coeffCast((-36 * v2 - 83 * v3) >> 8);
			stb[(y + 2) * CU_SIZE + x] = coeffCast((64 * (v0 - v1)) >> 8);
			stb[(y + 3) * CU_SIZE + x] = coeffCast((83 * v2 - 36 * v3) >> 8);
		}
	}
}
...
{
	// Process the luma components
	tuInverse(stu->hscu->scbLuma, CU_SIZE / 8, CU_SIZE);
	tuInverse(stu->vscu->scbLuma, CU_SIZE, CU_SIZE / 8);

	// Process the chroma components
	if (chromaSub == CHROMA_444)
	{
		tuInverse(stu->hscu->scbChroma1, CU_SIZE / 8, CU_SIZE);
		tuInverse(stu->hscu->scbChroma2, CU_SIZE / 8, CU_SIZE);
		tuInverse(stu->vscu->scbChroma1, CU_SIZE, CU_SIZE / 8);
		tuInverse(stu->vscu->scbChroma2, CU_SIZE, CU_SIZE / 8);
	}
	else if (chromaSub == CHROMA_420)
	{
		tuInverse(stu->hscu->scbChroma1, CU_SIZE / 16, CU_SIZE / 2);
		tuInverse(stu->hscu->scbChroma2, CU_SIZE / 16, CU_SIZE / 2);
		tuInverse(stu->vscu->scbChroma1, CU_SIZE / 2, CU_SIZE / 16);
		tuInverse(stu->vscu->scbChroma2, CU_SIZE / 2, CU_SIZE / 16);
	}
}
...
void tuInverse(s16 *stb, u8 width, u8 height)
{
	...
	for (u8 y = 0; y < height; y++)
	{
		for (u8 x = 0; x < width; x += 4)
		{
			v0 = 64 * ((s32)stb[y * CU_SIZE + x + 0] + (s32)stb[y * CU_SIZE + x + 2]);
			v1 = 64 * ((s32)stb[y * CU_SIZE + x + 0] - (s32)stb[y * CU_SIZE + x + 2]);
			v2 = -36 * (s32)stb[y * CU_SIZE + x + 1] + 83 * (s32)stb[y * CU_SIZE + x + 3];
			v3 = -83 * (s32)stb[y * CU_SIZE + x + 1] - 36 * (s32)stb[y * CU_SIZE + x + 3];
			stb[y * CU_SIZE + x + 0] = coeffCast((v0 - v3) >> 7);
			stb[y * CU_SIZE + x + 1] = coeffCast((v1 - v2) >> 7);
			stb[y * CU_SIZE + x + 2] = coeffCast((v1 + v2) >> 7);
			stb[y * CU_SIZE + x + 3] = coeffCast((v0 + v3) >> 7);
		}
	}
	...
	for (u8 y = 0; y < CU_SIZE / 16; y += 4)
	{
		...
		{
			v0 = 64 * ((s32)stb[(y + 0) * CU_SIZE + x] + (s32)stb[(y + 2) * CU_SIZE + x]);
			v1 = 64 * ((s32)stb[(y + 0) * CU_SIZE + x] - (s32)stb[(y + 2) * CU_SIZE + x]);
			v2 = -36 * (s32)stb[(y + 1) * CU_SIZE + x] + 83 * (s32)stb[(y + 3) * CU_SIZE + x];
			v3 = -83 * (s32)stb[(y + 1) * CU_SIZE + x] - 36 * (s32)stb[(y + 3) * CU_SIZE + x];
			stb[(y + 0) * CU_SIZE + x] = pixelCast((v0 - v3) >> 12);
			stb[(y + 1) * CU_SIZE + x] = pixelCast((v1 - v2) >> 12);
			stb[(y + 2) * CU_SIZE + x] = pixelCast((v1 + v2) >> 12);
			stb[(y + 3) * CU_SIZE + x] = pixelCast((v0 + v3) >> 12);
		}
	}
}
